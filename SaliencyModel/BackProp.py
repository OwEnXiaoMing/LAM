...
def Path_gradient(numpy_image, model, attr_objective, path_interpolation_func, cuda=False):
    ...
    if cuda:
        model = model.cuda()
    cv_numpy_image = np.moveaxis(numpy_image, 0, 2)#3 64 64 -> 64 64 3
    image_interpolation, lambda_derivative_interpolation = path_interpolation_func(cv_numpy_image)#50 3 64 64
    grad_accumulate_list = np.zeros_like(image_interpolation)
    result_list = []
    for i in range(image_interpolation.shape[0]):
        img_tensor = torch.from_numpy(image_interpolation[i])#3 64 64 cpu
        img_tensor.requires_grad_(True)
        if cuda:
            result = model(_add_batch_one(img_tensor).cuda())#1 3 256 256 cuda
            target = attr_objective(result)#
            target.backward()
            grad = img_tensor.grad.cpu().numpy()#numpy 3 64 64
        ...
        result_list.append(result)#[1,3,256,256]
    res = [e.to('cpu').detach() for e in result_list]
    results_numpy = np.asarray(res)
